...
from app.core.capabilities import CAPABILITY_ID_PHOTO, capabilities
...
from app.services.tools import id_photo as id_photo_service
...
def _get_id_photo_status() -> dict[str, object]:
    """读取证件照模型状态并同步工具目录可用性。"""
    status = id_photo_service.get_model_management_status()
...
def delete_model(body: ModelRequest) -> dict[str, object]:
    ...
    if body.model_id == "id-photo":
        status = id_photo_service.delete_id_photo_model()
