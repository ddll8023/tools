...
- 当前业务包含 PDF、Markdown、EPUB、Word、图片、证件照、二维码、工具目录和 MinerU 模型管理。
...
6. 保留 PDF 深度解析的进程池、模型下载的单任务限制和取消/中断状态。
...
| `modules/model_management` | MinerU 模型状态、下载、取消和中断恢复 | 处理用户业务文件 |
...
| 证件照模型资源 | 打包资源或用户数据目录 | `modules/id_photo` | 随应用资源或用户数据生命周期存在 |
...
后端只承担临时任务文件和模型缓存的本地生命周期；不建立用户文件库。临时任务过期清理由 Task Storage 负责，模型缓存是否清理属于模型管理或应用卸载流程，不在业务转换接口中隐式删除。
...
| 模型管理 | `/api/v1/settings/models/status`、`/download`、`/progress`、`/cancel` | POST | 空请求或 `ModelRequest` | `ModelStatusResponse` / `ModelActionResponse` |
